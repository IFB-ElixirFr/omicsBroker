observeEvent(input$loadProjectsSpace, {
  if(input$omicsBroker_user == "toto" & input$omicsBroker_pw == "toto"){
    appReac$login = T
    sendSweetAlert(
      session = session,
      title = "Success !",
    ...
    sendSweetAlert(
      session = session,
      title = "Ops!",
      text = "your username and/or password are incorrect",
      type = "error"
    )
  }
})
...
output$dataTable_adminProjects = renderRHandsontable({
  ...
  if(!is.null(appReac$login) & appReac$login){
    DF = data.frame(Title = c("Etude de l'impact du changement de concentration en fer chez la levure C. glabrata",
                    ...
                    Status = c("In progress",
                               "Ready to publish",
                               "In progress"),
                    Load = c('<a id="btn_1" class="btn btn-primary" href="#" role="button" onclick=\'Shiny.onInputChange("select_button",  this.id)\'>Load</a>',
                             '<a id="btn_2" class="btn btn-primary" href="#" role="button" onclick=\'Shiny.onInputChange("select_button",  this.id)\'>Load</a>',
                             '<a id="btn_3" class="btn btn-primary" href="#" role="button" onclick=\'Shiny.onInputChange("select_button",  this.id)\'>Load</a>'),
...
observeEvent(input$select_button, {
  ...
  updateTextInput(session, "projectSet_title",value =  "Title after load")
  updateTextInput(session, "projectSet_alias",value =  createAlias("Title after load"))
  updateTextAreaInput(session,"projectSet_description", value = "Description after load")
  updateTextAreaInput(session,"projectSet_submitter", value = "John Doe")

  sendSweetAlert(
    session = session,
    title = "Success !",
    text = "The project was successfully loaded !",
    type = "success"
  )
})
